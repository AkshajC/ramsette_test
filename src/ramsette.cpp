...
// const float maxMotor = 200;
// const float ratio = 450/200;
...
auto generator = squiggles::SplineGenerator(
  ...
  std::make_shared<squiggles::TankModel>(ROBOT_WIDTH, constraints), 10);
    ...
    for(std::size_t i = 0; i < pathSize; i++){
        ...
        leftMotors = (leftMotorVelocities/ratio)*(std::fabs(leftMotorVelocities)/leftMotorVelocities);
        rightMotors = (rightMotorVelocities/ratio)*(std::fabs(rightMotorVelocities)/rightMotorVelocities);
